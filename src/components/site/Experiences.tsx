        ...
        <Reveal key={experience.id} as="li" delay={Math.min(i * 0.07, 0.35)}>
          <article className="group relative flex h-full flex-col overflow-hidden rounded-[1.5rem] border border-white/8 bg-charcoal/60">
            ...
            <div className="flex flex-1 flex-col p-7">
              <h3 className="font-display text-[1.1475rem] text-warm transition-colors group-hover:text-champagne">
                {experience.title}
              </h3>
              <p className="mt-3 flex-1 text-[0.748rem] leading-relaxed text-muted">
                {experience.description}
              </p>

              {experience.price_note ? (
                <p className="mt-5 font-ui text-[0.612rem] uppercase tracking-[0.16em] text-gold/85">
                  {experience.price_note}
                </p>
              ) : null}
